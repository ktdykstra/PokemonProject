...
# #### 1. Gather matches and chunk text
# #### 2. Gather match metadata
# #### 3. Gather turn data
# #### 4. Aggregate turn data
# #### 5. Aggregate all data
# 
# #### Putting data into pandas (class/object for this data structure?)
# 
# #### Advanced methods and output data structures
# 
# #### test code. Read in method (username)
...
def gather_matches(username, game_type):
    ...
    body_logs, head_logs, tail_logs, turns, turn_count = get_logs(base_db)
...
def get_logs(df):
    ...
    for i in df.match_id:
        ...
        json=requests.get(match_url).json()
        log=json["log"]
...
def run_all_steps_metrics(sample_username, sample_game_type):
    
    ## Gather matches from Showdown
    
    MATCH_DB=gather_matches(sample_username, sample_game_type)
...
# library, meta, hero_pairs_db, villain_pairs_db, hero_comps_db, villain_comps_db =run_all_steps_metrics(sample_username, sample_game_type)
